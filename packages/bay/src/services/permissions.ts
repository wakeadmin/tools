 ...
 */
export async function allows(to: string[] | string, type: PermissionCheckType = 'OR') {
 ...
 * @param type 检查的类型, 支持 AND 或 OR，在传递多个权限标识符时有用，AND 表示必须拥有全部权限，OR 表示拥有其中一个即可
 */
export function allowsSync(to: string[] | string, type: PermissionCheckType = 'OR') {
  const roles = Array.isArray(to) ? to : [to];
  const bayModel = getInject('DI.bay.BayModel');
